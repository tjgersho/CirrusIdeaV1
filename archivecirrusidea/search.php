...
$root = realpath($_SERVER["DOCUMENT_ROOT"]);  
 ...
 $dbc = mysqli_connect(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
  ...
  function build_query($user_search, $sort) {
    $search_query = "SELECT * FROM creativebrainpower";
  ...
  function generate_sort_links($user_search, $sort) {
    ...
    switch ($sort) {
    case 1:
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=2">Link</a></b></td><td><b>Description</b></td>';
  
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=5">Date Posted</a></b></td>';
      break;
    case 3:
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=1">Link</a></b></td><td><b>Description</b></td>';
   
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=3">Date Posted</a></b></td>';
      break;
    case 5:
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=1">Link</a></b></td><td><b>Description</b></td>';
     
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=6">Date Posted</a></b></td>';
      break;
    default:
      $sort_links .= '<td><b><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=1">Link</a></b></td><td><b>Description</b></td>';
...
<br />
  <form method="post" action="search.php" style="text-align:center;">
    <label for="usersearch">CirrusIdea Search: </label>
...
if($show==1){ 
    ...
    echo '<tr>';
    echo '<td valign="top" width="20%"><a href="http://www.cirrusidea.com' . $row['file_id'] . '">' .basename($row['file_id']). '</a></td>';
    echo '<td valign="top" width="60%">' . substr($row['description'], 0, 100) . '...</td>';
